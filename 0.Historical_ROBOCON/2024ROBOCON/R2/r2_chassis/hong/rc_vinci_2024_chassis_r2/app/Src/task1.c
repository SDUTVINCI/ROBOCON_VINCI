...
uint8_t Mapan_move_postion(int x_of_3port, int y_of_3port, fp32 omega) // 世界坐标系
{
	move_mode = 2;

	if ((fabs(yaw - omega) < 0.8) && ((abs((int)(x - x_of_3port))) < 10) && ((abs((int)(y - y_of_3port))) < 10))
	{
		world_vx = 0;
		world_vy = 0;
		// world_vomega = 0;
...
int x_point_mapan[7] = {0, -2600, 400, 200, 0}; //[0]  初始零点
int y_point_mapan[7] = {0, 0, -3000, -2800, 0};
...
void daokuang_task(void const *argument)
{
	...
	for (;;)
	{
		...
		vTaskResume(mapanmoveHandle);

		while (if_mapan_move_point_task_finish == 0)
		{
			osDelay(10);
		}
		osDelay(10);
		...
		// if_mapan_move_point_task_finish = 0; // 接收到下一个底盘任务信息时 码盘任务标志位才清零
