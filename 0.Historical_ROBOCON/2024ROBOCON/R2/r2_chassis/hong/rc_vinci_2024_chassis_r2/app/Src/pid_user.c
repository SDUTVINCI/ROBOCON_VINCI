...
float motor_position_chassis_L1S[3] = {6, 0, 0};
float motor_position_chassis_mapan[3] = {3, 0, 0.1};     // 1 0 1.5
float motor_position_chassis_omega1[3] = {0.14, 0, 0.2}; //{0.5,0,0.02};
...
void PID_devices_Init(void)
{
	...
	{
		PID_init(&pid_v_chassis_3508[i], PID_POSITION, motor_speed_3508_pid, 10000, 600);      // 3508  speed
	...
	PID_init(&pid_position_chassis_x, PID_POSITION, motor_position_chassis_L1S, 1600, 300); // 激光测距定位
	PID_init(&pid_position_chassis_y, PID_POSITION, motor_position_chassis_L1S, 1600, 300);
	PID_init(&pid_position_chassis_omega1, PID_POSITION, motor_position_chassis_omega1, 10, 3);
	PID_init(&pid_position_chassis_omega2, PID_POSITION, motor_position_chassis_omega2, 10, 3);
	PID_init(&pid_position_chassis_jiaozhun, PID_POSITION, motor_position_chassis_jiaozhun, 8, 3);
...
{
	PID_calc(&pid_position_chassis_x, x, set_position);
	return pid_position_chassis_x.out;
}
fp32 PID_position_mapan_y(fp32 set_position) // position
{
	PID_calc(&pid_position_chassis_y, y, set_position);
...
fp32 PID_position_Mapan_y(int set_position) // position
{
	...
	PID_calc(&pid_position_mapan_y, y, set_position);
...
fp32 PID_position_Mapan_x(int set_position) // position
{
	...
	PID_calc(&pid_position_mapan_x, x, set_position);
