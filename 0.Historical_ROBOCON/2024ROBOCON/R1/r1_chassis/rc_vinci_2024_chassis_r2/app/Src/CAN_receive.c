...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
   ...
   && (rx_header.StdId <  FEEDBACK_ID_BASE + MOTOR_MAX_NUM))                  // judge the can id
  {
    ...
    motor_info[index].temp           =   rx_data_2[6];
	  	  ...
	  	  if( index==0)//对对应的6020设置0位置的偏移量
	  {
		 ...
		 motor_info[index].rotor_angle +=8191*8/12;
  }
	  	  	  if( index==2)//对对应的6020设置0位置的偏移量
	  {
		
		 motor_info[index].rotor_angle +=8191*3/12;
  }
 ...
 void gm3508_cmd(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4)//can1 gm3508
{
