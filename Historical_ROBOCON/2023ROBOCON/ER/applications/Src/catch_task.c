...
void catch_task(void const * argument)
{
	while(1)
	{
		...
		if(rc_ctrl.rc.s[0] == 2)
		{
			if(rc_ctrl.rc.ch[1] > 500 && ABS(rc_ctrl.rc.ch[0]) < 200)
			{
				HAL_GPIO_WritePin(Clamp_GPIO_Port,Clamp_Pin,GPIO_PIN_SET);
			}
			else if(rc_ctrl.rc.ch[1] < -500 && ABS(rc_ctrl.rc.ch[0]) < 200)
			{
				HAL_GPIO_WritePin(Clamp_GPIO_Port,Clamp_Pin,GPIO_PIN_RESET);
			}
		}
...
void push_task(void const * argument)
{
	while(1)
	{
		...
		osDelay(750);
		push_deg_angle = 98.0f;
		*Push_Angle_Target = -8192.0f * 100.0f * push_deg_angle / 360.0f;
...
void can2_cmd_task(void const * argument)
{
	...
	while(1)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			CMD_Catch_Current_Target[i] = PID_call_2(CMD_Catch_Angle_Target[i], i+5);
		}
		...
		CMD_Push_Current_Target[0] = PID_call_2(CMD_Push_Angle_Target[0], 7);
		
		CAN2_CMD_2(CMD_Catch_Current_Target[0], CMD_Catch_Current_Target[1], CMD_Push_Current_Target[0], 0);
	}
}
